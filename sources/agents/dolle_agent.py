...
class DolleAgent(Agent, AssociativeAgent):
    ...
    def take_decision(self, s, orientation):
        ...
        if (decision_arbi == 0 or (self.lesion_PFC == True and self.lesion_HPC == True)):
            allo_a = self.softmax_selection(Q=Q, inv_temp=self.DLS.inv_temp)
        if (decision_arbi == 1 or (self.lesion_PFC == True and self.lesion_DLS == True)):
            allo_a = self.softmax_selection(Q=Q, inv_temp=self.HPC.inv_temp)
        if self.lesion_HPC == True and self.lesion_DLS == True:
            allo_a = random.choice(Q)
    ...
    def compute_Q(self, state_idx):
        ...
        if (decision_arbitrator == 0 or (self.lesion_PFC == True and self.lesion_HPC == True)):
            Q = Q_allo
        if (decision_arbitrator == 1 or (self.lesion_PFC == True and self.lesion_DLS == True)):
            Q = Q_sr
        if self.lesion_HPC == True and self.lesion_DLS == True:
            Q = np.zeros(6)
