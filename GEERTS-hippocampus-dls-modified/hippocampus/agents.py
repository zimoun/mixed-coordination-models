...
class LandmarkLearningAgent(object):
    ...
    def __init__(self, environment=Environment(), learning_rate=.07, gamma=.9, eta=.03, beta=10):
        """
...
class LandmarkCells(object):
    ...
    def compute_response(self, distance, angle):
        ...
        #MODIF
        if (int(distance),int(angle)) in self.responsees:
            response = self.responsees[(int(distance),int(angle))]
        else:
            response = np.array([f.pdf([distance, angle]) * np.sqrt((2*np.pi)**2 * np.linalg.det(f.cov)) for f in self.receptive_fields])
        # stores observed signal for each angle-distance couple, to dramatically increase computational performances
        self.responsees[(int(distance),int(angle))] = response
